    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {bookingModalService && (
        <div className="fixed inset-0 z-50 bg-neutral-950/60 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-5 text-neutral-900">
            ...
            <div className="p-4 bg-neutral-900 text-white flex items-center justify-between">
              <div>
                ...
                <div className="text-[10px] text-neutral-300 flex items-center gap-1.5 mt-0.5">
                  <span>{bookingModalService.provider.name}</span>
                  <span className="bg-emerald-600 text-white text-[9px] font-extrabold px-1.5 py-0.2 rounded">
                    Verified Provider
                  </span>
                </div>
              </div>
              <button onClick={() => setBookingModalService(null)} className="p-1.5 rounded-lg hover:bg-neutral-800 text-neutral-400 min-h-[44px] min-w-[44px] flex items-center justify-center">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              {isBookingSuccess ? (
                <div className="py-8 text-center space-y-3">
                  ...
                  <p className="text-xs text-neutral-500">
                    Technician assigned for Vikram (Apt 204, Tower A). Track in My Requests.
                  </p>
                </div>
              ) : (
                <>
                  {/* Provider Verification & Trust Box (Requirement 26) */}
                  <div className="bg-neutral-50 p-3.5 rounded-xl border border-neutral-200 text-xs space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-neutral-900 flex items-center gap-1.5">
                        <ShieldCheck className="w-4 h-4 text-emerald-600" />
                        {bookingModalService.provider.name}
                      </span>
                      <span className="text-[11px] font-bold text-amber-700">★ 4.9 (148 jobs)</span>
                    </div>
                    <div className="text-[11px] text-neutral-600 space-y-0.5">
                      <div>Verification: <strong className="text-neutral-900">Govt ID & Police Verification Complete</strong></div>
                      <div>Cancellation Policy: <strong className="text-neutral-900">Free cancellation up to 2 hrs before slot</strong></div>
                      <div>Dispute Handling: <strong className="text-neutral-900">24/7 Estate Helpdesk Dispute Escrow</strong></div>
                    </div>
                  </div>
                        ...
                        {bookingModalService.tiers.map((tier) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full p-3 rounded-xl border text-left flex items-center justify-between transition-all min-h-[44px] ${
                                isSelected
                            ...
                            >
                              ...
                              <div className="text-xs font-bold text-neutral-900 font-mono">₹{tier.price}</div>
                            </button>
                          );
                        })}
                      </div>
                  ...
                  {/* Dynamic Date Picker (Requirement 1) */}
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-neutral-900 block">Select Date</label>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      {["Today · 12 Aug 2026", "Tomorrow · 13 Aug 2026"].map((dateStr) => (
                        <button
                          key={dateStr}
                          onClick={() => setSelectedDate(dateStr)}
                          className={`py-2.5 px-2 rounded-xl border text-center font-bold text-[11px] transition-colors min-h-[44px] ${
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      {["09:00 AM - 10:00 AM", "02:00 PM - 03:00 PM", "05:00 PM - 06:00 PM", "07:00 PM - 08:00 PM"].map((slot) => (
                        <button
                          ...
                          className={`py-2.5 rounded-xl border text-center font-semibold text-xs transition-colors min-h-[44px] ${
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl bg-neutral-900 hover:bg-neutral-800 text-white font-bold text-xs transition-colors shadow-2xs min-h-[44px]"
                  >
