...
export const SecurityVisitorsModule: React.FC = () => {
  const { visitorPasses, addVisitorPass, revokeVisitorPass, userRole, setUserRole } = useSociety();

  const [activeTab, setActiveTab] = useState<"visitors" | "parcels" | "helpers">("visitors");
  const [isPassModalOpen, setIsPassModalOpen] = useState(false);
  const [showGateConsoleDemo, setShowGateConsoleDemo] = useState(false);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-neutral-200">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowGateConsoleDemo(!showGateConsoleDemo)}
            className="border border-neutral-300 hover:bg-neutral-100 text-neutral-800 font-bold text-xs px-3.5 py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-colors min-h-[44px]"
          >
            <ScanLine className="w-4 h-4 text-indigo-600" />
            <span>{showGateConsoleDemo ? "Hide Gate Console" : "Gate Scanner Simulator"}</span>
          </button>
      ...
      {/* SECURITY GUARD CONSOLE (Role-Based Display & Simulator) */}
      {(userRole === "security" || showGateConsoleDemo) && (
        <div className="p-5 bg-neutral-900 text-white rounded-2xl space-y-4 border border-neutral-800 shadow-md">
          ...
          <form onSubmit={handleVerifyGateCode} className="flex gap-2">
            <input
              type="text"
              placeholder="Scan QR token or enter pass code (e.g. PASS-2026-X892)..."
              value={scanCodeInput}
              onChange={(e) => setScanCodeInput(e.target.value)}
              className="flex-1 px-4 py-2.5 text-xs rounded-xl bg-neutral-800 text-white border border-neutral-700 font-mono focus:outline-none focus:border-white min-h-[44px]"
            />
            <button
              type="submit"
              className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-colors min-h-[44px]"
            >
              Verify Pass Code
            </button>
          </form>
            ...
            <div className={`p-3.5 rounded-xl text-xs flex items-center justify-between ${
              ...
              {scanResult.valid ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>INVALID OR EXPIRED PASS CODE. Gate Entry Denied.</span>
                </div>
              )}
            </div>
          )}
      ...
      <div className="flex items-center gap-2 border-b border-neutral-100 pb-2 text-xs">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-bold transition-all flex items-center gap-2 min-h-[44px] ${
            activeTab === "visitors"
            ...
            {visitorPasses.map((vis) => (
              <div key={vis.id} className="saas-card p-5 space-y-3 flex flex-col justify-between">
                ...
                <div className="pt-2 border-t border-neutral-100 flex items-center justify-between text-xs">
                  <span className="text-neutral-400 font-mono">Unit: Apt 204</span>
                  {vis.status === "Pre-Approved" && (
                    <button
                      onClick={() => revokeVisitorPass(vis.id)}
                      className="text-xs font-bold text-red-600 hover:underline flex items-center gap-1 min-h-[36px]"
                    >
                ...
                <div>
                  {prc.status === "Entry Approved" ? (
                    ...
                    <button
                      ...
                      className="bg-neutral-900 text-white hover:bg-neutral-800 text-xs font-semibold px-3.5 py-2 rounded-xl transition-colors shadow-2xs min-h-[44px]"
                    >
                      Approve Entry
                    </button>
                  )}
      ...
      {isPassModalOpen && (
        <div className="fixed inset-0 z-50 bg-neutral-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-neutral-200 animate-in zoom-in-95 duration-200 relative text-neutral-900">
            <button
            ...
            {!passCreatedCode ? (
              <form onSubmit={handleCreatePass} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-neutral-50 border border-neutral-300 px-3 py-2.5 rounded-xl text-xs text-neutral-900 focus:outline-none focus:border-neutral-900"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-neutral-50 border border-neutral-300 px-3 py-2 rounded-xl text-xs text-neutral-900"
                    />
                  </div>
                </div>
              ...
              <div className="text-center space-y-4 py-2">
                ...
                <button
                  onClick={() => setIsPassModalOpen(false)}
                  className="w-full bg-neutral-100 hover:bg-neutral-200 text-neutral-900 font-bold text-xs py-3 rounded-xl transition-colors min-h-[44px]"
                >
                  Done & Share Pass
                </button>
              </div>
